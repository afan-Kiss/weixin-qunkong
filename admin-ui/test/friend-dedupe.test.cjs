...
test('friend targets are deduplicated across tasks and instances of the same WeChat account', () => {
  ...
  assert.deepEqual(storage.createTask(task('task-2'), [item('item-2', 'instance-b', 'member-one')]), { inserted: 0, duplicates: 1 })
  assert.deepEqual(storage.createTask(task('task-3'), [item('item-3', 'instance-b', 'member-two')]), { inserted: 1, duplicates: 0 })
  assert.equal(storage.listTasks().some((row) => row.id === 'task-2'), false)
  assert.equal(storage.listTasks().find((row) => row.id === 'task-3').total, 1)

  // 取消未执行任务后释放占用，允许重新创建
  assert.deepEqual(storage.cancelTask('task-1'), { released: 1 })
