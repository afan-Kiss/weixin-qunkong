...
test('saveEvent and snapshot sync persist latest join members with frequent-ready statuses query', () => {
  ...
  const statuses = storage.listFriendAddStatuses(['wxid_join'])
  assert.equal(statuses.wxid_join.taskStatus, 'COOLING_DOWN')
  assert.match(statuses.wxid_join.error, /频繁/)
})
...
test('groups page uses joinRows as latest-member source and avoids frequent double count', () => {
  ...
  assert.match(page, /function joinToMember/)
