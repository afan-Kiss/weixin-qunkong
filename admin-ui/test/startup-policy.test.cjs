...
test('application uses an operating system single-instance lock', () => {
  ...
  assert.match(source, /migrateLegacyBusinessDataIfNeeded/)
  assert.match(source, /%LOCALAPPDATA%\\\\WXQK|LOCALAPPDATA/)
  const bizPos = source.indexOf('migrateLegacyBusinessDataIfNeeded')
  const initPos = source.indexOf('initStorage(app.getPath')
  assert.ok(bizPos > 0 && initPos > bizPos, 'business DB migration must run before initStorage')
  ...
  assert.match(pathsSrc, /allowUserDataDirOverride/)
  assert.match(pathsSrc, /isPackaged/)
