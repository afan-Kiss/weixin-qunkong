...
test('chat add friend page, route, sidebar and main wiring exist', () => {
  ...
  assert.match(page, /status === 'PENDING' \|\| row\.status === 'TASKED'/)
  assert.doesNotMatch(page, /已经创建过加好友任务，已从待创建中移除/)
  assert.match(page, /watch\(selectedInstanceId, \(next, prev\)/)
  assert.match(page, /restoreCandidateSelection/)
  assert.match(page, /scheduleRefreshCandidates/)
  assert.match(page, /candidatesRefreshSeq/)
  assert.match(page, /reserve-selection/)
  assert.match(page, /row-key="id"/)
  assert.match(page, /onCandidateSelectionChange/)
  assert.match(page, /restoringSelection/)
  assert.match(page, /reloadGroupsForInstance/)
  assert.match(page, /persistDraft/)
  assert.match(page, /applyRoomSelectionFromIds/)
  assert.match(page, /rememberedRoomIds/)
  assert.match(page, /queuePersistDraft/)
  assert.match(page, /restoreRoomIds/)
  assert.match(page, /matchedByRoom/)
  assert.match(page, /previousRule\?\.accountWxid/)
  assert.match(page, /refreshDirectory\(\[id\], \{ force: true \}\)/)
  assert.match(page, /status === 'ONLINE' && prev !== 'ONLINE'/)
