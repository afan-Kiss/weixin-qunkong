...
const dashboardPendingTasks = computed(() => rawTasks.value.filter((item) => ['WAITING_CONFIRMATION', 'QUEUED', 'RUNNING', 'PAUSED', 'COOLING_DOWN', 'BLOCKED_API_UNVERIFIED'].includes(String(item.status))).map((item) => { const total = Number(item.total || 0); const done = Number(item.success || 0) + Number(item.failed || 0) + Number(item.skipped || 0); return { ...item, targets: total, progress: total ? Math.round(done / total * 100) : 0, interval: '全局设置', nextRun: '-' } }))
