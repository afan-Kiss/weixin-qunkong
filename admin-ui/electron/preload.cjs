...
contextBridge.exposeInMainWorld('wxControl', {
  ...
  confirmTask: (id) => ipcRenderer.invoke('tasks:confirm', id),
  ...
  remoteStart: (opts) => ipcRenderer.invoke('remote:start', plainIpcValue(opts || {})),
  remoteStop: () => ipcRenderer.invoke('remote:stop'),
  openRemoteConsole: (token, baseUrl) => ipcRenderer.invoke('remote:open-console', token, baseUrl),
