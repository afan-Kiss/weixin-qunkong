...
<div id="loginScreen">
  <div class="card login-card">
    <div class="brand-row" style="margin-bottom:16px">
      ...
      <div><div class="title">微信群控管理平台</div><div class="sub">远程桌面与设备控制中心</div></div>
    </div>
...
/* ===== state ===== */
const ROUTES = [
  { id:'dashboard', label:'总览', title:'控制台总览', desc:'查看在线客户端、远程桌面与微信会话数据。' },
  ...
  { id:'desktop', label:'远程桌面', title:'远程桌面', desc:'复用发财888：选择客户端后静默查看并控制桌面。' },
...
(function bootstrapAdminTokenFromHash(){
  try {
    ...
    if (tok) {
      ...
      history.replaceState(null, '', location.pathname + location.search + '#/desktop');
    }
  } catch (_) {}
})();
const state = {
...
let deskStatsTimer = null;
let deskLastFrameSig = '';
let deskLastBytes = 0;
let deskLastStatsTs = 0;
let deskFrameBytes = 0;
let deskFrameCount = 0;
let deskLastFrameAt = 0;
let deskLastLiveFrameAt = 0;
let deskLastFrameStamp = '';
let deskStallKickAt = 0;
let deskStallKickStreak = 0;
let deskPrevFrameBytes = 0;
let deskPrevFrameCount = 0;
let deskPrevFrameTs = 0;
let deskDeltaLastSeq = 0;
let deskDeltaKeySeq = 0;
let deskDeltaW = 0;
let deskDeltaH = 0;
...
/**
 * Pure helpers for admin remote-desktop canvas continuity.
 * Loaded by frontend tests; inlined into admin_ui.py for the browser SPA.
 */

function shouldApplyPollFrame({
  wsOpen,
  lastLiveFrameAt,
  now,
  hasRealtimeBasis,
  ...
  const liveAgeOk =
    !!lastLiveFrameAt && Number(now) - Number(lastLiveFrameAt) < Number(liveGraceMs || 3000);
  if (wsOpen && liveAgeOk && hasRealtimeBasis) {
    return false;
  }
  return true;
}

function nextFrameGeneration(current) {
  return (Number(current) || 0) + 1;
}

function isFrameCallbackCurrent({
  generation,
  expectedGeneration,
  session,
  expectedSession,
  ...
  if (Number(generation) !== Number(expectedGeneration)) return false;
  if (session !== expectedSession) return false;
  if (String(clientId || '') !== String(expectedClientId || '')) return false;
  return true;
}
...
/** Stall-kick cooldown with exponential backoff (mirrors deskFrameGuard.js). */
function stallKickCooldownMs(streak, opts) {
  const o = opts || {};
  const s = Math.max(0, Math.min(8, Math.floor(Number(streak) || 0)));
  const base = Math.max(1000, Number(o.baseMs) || 5000);
  const cap = Math.max(base, Number(o.maxMs) || 60000);
  return Math.min(cap, base * Math.pow(2, s));
}
function shouldStallKick(opts) {
  const o = opts || {};
  const age = Number(o.liveAgeMs);
  const liveStallMs = Math.max(1000, Number(o.liveStallMs) || 5000);
  const stalled = !!o.waitingKeyframeTooLong || !Number.isFinite(age) || age >= liveStallMs;
  if (!stalled) return false;
  const cooldown = stallKickCooldownMs(o.streak || 0);
  const last = Number(o.lastKickAt) || 0;
...
/* ===== desktopWS / WebRTC ===== */
async function deskWsURL(clientId) {
  const ticketData = await api('/api/admin/ws-ticket', { method:'POST', body:JSON.stringify({ clientId }) });
  const u = new URL(location.href);
  u.protocol = (u.protocol === 'https:') ? 'wss:' : 'ws:';
  u.pathname = publicApiRoot() + '/api/ws/viewer';
  u.search = 'clientId=' + encodeURIComponent(clientId) + '&ticket=' + encodeURIComponent(ticketData.ticket || '');
  u.hash = '';
  return u.toString();
}
function getDeskQuality() {
  const sel = document.getElementById('deskQuality');
  return (sel && sel.value) ? sel.value : 'auto';
}
function resetDeskStatsDisplay() {
  ['deskStatRtt', 'deskStatBitrate', 'deskStatDrop', 'deskStatIce'].forEach(id => {
    const el = document.getElementById(id);
    if (el) el.textContent = '—';
  });
}
function noteDeskFrameBytes(n, opts) {
  const size = Number(n) || 0;
  if (size > 0) deskFrameBytes += size;
  deskFrameCount += 1;
  deskLastFrameAt = Date.now();
  const cached = !!(opts && opts.cached);
  const stamp = opts && opts.stamp != null ? String(opts.stamp) : '';
  if (!cached) {
    if (!stamp || stamp !== deskLastFrameStamp) {
      deskLastLiveFrameAt = deskLastFrameAt;
      deskLastFrameStamp = stamp || deskLastFrameStamp;
...
function stopDeskPing() {
  if (deskPingTimer) {
    clearInterval(deskPingTimer);
    deskPingTimer = null;
  }
}
function startDeskPing(ws) {
  stopDeskPing();
  deskPingTimer = setInterval(() => {
    if (!deskWS || deskWS !== ws || deskWS.readyState !== 1) return;
    try { deskWS.send(JSON.stringify({ type: 'ping', t: Date.now() })); } catch (_) {}
  }, 25000);
}

let deskWsReconnectTimer = 0;
let deskWsReconnectAttempt = 0;
function clearDeskWsReconnect() {
  if (deskWsReconnectTimer) {
    clearTimeout(deskWsReconnectTimer);
    deskWsReconnectTimer = 0;
  }
}
function scheduleDeskWsReconnect(reason) {
  const clientId = state.desktopClientId;
  if (!clientId || deskConnecting) return;
  if (deskWsReconnectTimer) return;
  const attempt = Math.max(0, Number(deskWsReconnectAttempt) || 0);
  const delay = Math.min(20000, 1000 * Math.pow(2, Math.min(4, attempt)));
  deskWsReconnectAttempt = attempt + 1;
  const hint = document.getElementById('deskHint');
  if (hint) {
    hint.textContent = '实时通道断开，' + Math.round(delay / 1000) + 's 后自动重连…';
  }
  deskWsReconnectTimer = setTimeout(() => {
    deskWsReconnectTimer = 0;
    void reopenDeskViewerSocket(String(reason || 'auto'));
  }, delay);
}
function bindDeskViewerSocket(ws, clientId, session) {
  const hint = document.getElementById('deskHint');
  const quality = getDeskQuality();
  ws.onopen = () => {
    if (deskWS !== ws || session !== state.desktopSession) { try { ws.close(); } catch (_) {} return; }
    deskWsReconnectAttempt = 0;
    clearDeskWsReconnect();
    startDeskPing(ws);
    if (hint && !document.getElementById('deskStage')?.classList.contains('has-frame')) {
  ...
  ws.onmessage = ev => {
    if (deskWS !== ws || session !== state.desktopSession) return;
    try {
      const msg = JSON.parse(ev.data);
      if (msg.type === 'pong') return;
      if (msg.type === 'frame' && msg.image) {
        showDeskFrame(msg.image, msg.t || '', {
          cached: !!msg.cached,
          seq: msg.seq,
          keySeq: msg.keySeq
        });
  ...
  ws.onclose = () => {
    if (deskWS !== ws) return;
    stopDeskPing();
    if (state.desktopClientId === clientId && session === state.desktopSession) {
      scheduleDeskWsReconnect('ws-close');
    }
  };
  ws.onerror = () => {
    if (deskWS !== ws) return;
    if (state.desktopClientId === clientId && session === state.desktopSession) {
      scheduleDeskWsReconnect('ws-error');
    }
  };
}
async function reopenDeskViewerSocket(reason) {
  const clientId = state.desktopClientId;
  if (!clientId || deskConnecting) return;
  if (deskWS && deskWS.readyState === 1) return;
  const session = state.desktopSession;
  stopDeskPing();
  if (deskWS) { try { deskWS.close(); } catch (_) {} deskWS = null; }
  deskNeedKeyframe = true;
  ...
  try {
    const ws = new WebSocket(await deskWsURL(clientId));
    deskWS = ws;
    bindDeskViewerSocket(ws, clientId, session);
  } catch (_) {
    scheduleDeskWsReconnect('reopen-throw');
  }
}
function setDeskStaleBanner(text, on) {
  const stage = document.getElementById('deskStage');
  const banner = document.getElementById('deskStaleBanner');
  if (!stage || !banner) return;
  if (on) {
    stage.classList.add('is-stale');
    banner.textContent = text || '画面已停住（缓存）';
  } else {
    stage.classList.remove('is-stale');
  }
}
function updateDeskStaleFromAges(opts) {
  const liveAge = Number(opts && opts.liveAgeMs) || 0;
  const shotAgeSec = Number(opts && opts.shotAgeSec);
  const clientOnline = opts && opts.clientOnline;
  const hasFrame = !!(document.getElementById('deskStage')?.classList.contains('has-frame'));
  if (!hasFrame) {
    setDeskStaleBanner('', false);
    return;
  }
  ...
  if (liveAge > 8000 || (Number.isFinite(shotAgeSec) && shotAgeSec > 12)) {
    const sec = Number.isFinite(shotAgeSec) && shotAgeSec > 0
      ...
      : Math.round(liveAge / 1000);
    setDeskStaleBanner('画面卡死约 ' + sec + 's — 正在尝试重拉推流…', true);
    return;
  }
  setDeskStaleBanner('', false);
}
function kickDesktopIfStalled() {
  const clientId = state.desktopClientId;
  if (!clientId || deskConnecting) return;
  // Viewer WS died: last frame freezes while agent heartbeat stays online.
  ...
  updateDeskStaleFromAges({ liveAgeMs: liveAge });
  const waitingKey = deskNeedKeyframe && deskNeedKeyframeSince && (now - deskNeedKeyframeSince) > 2500;
  if (!shouldStallKick({
    liveAgeMs: liveAge,
    waitingKeyframeTooLong: waitingKey,
    lastKickAt: deskStallKickAt,
    now,
    streak: deskStallKickStreak,
    liveStallMs: 5000
  })) return;
  deskStallKickAt = now;
  deskStallKickStreak = Math.min(8, (Number(deskStallKickStreak) || 0) + 1);
  bumpDeskFrameGeneration('stall-kick');
  deskNeedKeyframe = true;
  deskNeedKeyframeSince = now;
  deskDeltaInFlight = false;
  const hint = document.getElementById('deskHint');
  if (hint) {
    const waitSec = Math.round(stallKickCooldownMs(deskStallKickStreak) / 1000);
...
function stopDeskStats() {
  if (deskStatsTimer) {
    clearInterval(deskStatsTimer);
    deskStatsTimer = null;
  }
  deskLastBytes = 0;
  deskLastStatsTs = 0;
  deskPrevFrameBytes = 0;
  deskPrevFrameCount = 0;
  deskPrevFrameTs = 0;
}
function startDeskStats() {
  stopDeskStats();
  deskPrevFrameBytes = deskFrameBytes;
  deskPrevFrameCount = deskFrameCount;
  deskPrevFrameTs = Date.now();
  deskStatsTimer = setInterval(() => {
    const now = Date.now();
    const rttEl = document.getElementById('deskStatRtt');
    const brEl = document.getElementById('deskStatBitrate');
    const dropEl = document.getElementById('deskStatDrop');
    const iceEl = document.getElementById('deskStatIce');
    // JPEG/帧通道：根据收到的帧字节估算码率（远程桌面主路径）。
    const frameAge = deskLastFrameAt ? (now - deskLastFrameAt) : 99999;
    if (deskPrevFrameTs && deskFrameBytes >= deskPrevFrameBytes) {
      const dt = (now - deskPrevFrameTs) / 1000;
      if (dt > 0.4) {
        const kbps = Math.round(((deskFrameBytes - deskPrevFrameBytes) * 8) / dt / 1000);
        if (brEl) brEl.textContent = (kbps >= 0 ? kbps : 0) + ' kbps';
        const frames = deskFrameCount - deskPrevFrameCount;
        if (dropEl) {
          dropEl.textContent = frames > 0
            ? (Math.round((frames / dt) * 10) / 10) + ' fps'
            : (frameAge > 3500 ? '卡顿' : '0 fps');
        }
        deskPrevFrameBytes = deskFrameBytes;
        deskPrevFrameCount = deskFrameCount;
        deskPrevFrameTs = now;
    ...
    if (rttEl) {
      rttEl.textContent = frameAge < 3500
        ? Math.max(0, Math.round(frameAge)) + 'ms'
        : (deskLastFrameAt ? '超时' : '—');
    }
    if (iceEl) {
...
function bumpDeskFrameGeneration(reason) {
  deskFrameGeneration = nextFrameGeneration(deskFrameGeneration);
  deskPaintSession = state.desktopSession;
  deskPaintClientId = String(state.desktopClientId || '');
  deskDeltaInFlight = false;
  return deskFrameGeneration;
}
function deskCallbackCurrent(gen) {
  return isFrameCallbackCurrent({
    generation: gen,
    expectedGeneration: deskFrameGeneration,
    session: deskPaintSession,
    expectedSession: state.desktopSession,
    clientId: deskPaintClientId,
    expectedClientId: state.desktopClientId
  });
}
function clearDeskFrame() {
  const img = document.getElementById('deskImg');
  const canvas = document.getElementById('deskCanvas');
  const video = document.getElementById('deskVideo');
  const stage = document.getElementById('deskStage');
  bumpDeskFrameGeneration('clear');
  deskLastFrameSig = '';
  deskLastLiveFrameAt = 0;
  deskLastFrameStamp = '';
  deskStallKickAt = 0;
  deskStallKickStreak = 0;
  deskDeltaLastSeq = 0;
  ...
  if (stage) stage.classList.remove('has-frame', 'has-video', 'has-canvas', 'is-live');
  resetDeskStatsDisplay();
}
function drawDeskBitmap(bmp, t) {
  const canvas = document.getElementById('deskCanvas');
  const stage = document.getElementById('deskStage');
  const hint = document.getElementById('deskHint');
  ...
  deskDeltaW = bmp.width;
  deskDeltaH = bmp.height;
  deskNeedKeyframe = false;
  if (stage) {
    stage.classList.add('has-frame', 'has-canvas', 'is-live');
    stage.classList.remove('has-video');
  }
  if (hint) hint.textContent = '桌面画面 · ' + String(t || '刚刚') + '（单击全屏）';
  return true;
}
function showDeskCanvasFromData(image, t, opts) {
  ...
  if (!/^data:image\//i.test(src) && !/^https?:\/\//i.test(src)) return false;
  const cached = !!(opts && opts.cached);
  const forceApply = deskNeedKeyframe || !!(opts && opts.seq != null && !cached);
  const sig = src.length + ':' + src.slice(0, 64) + ':' + String(t || '') + ':' + (cached ? 'c' : 'l');
  if (!forceApply && sig === deskLastFrameSig) return true;
  deskLastFrameSig = sig;
  const gen = (!cached && opts && opts.seq != null)
    ? bumpDeskFrameGeneration('keyframe')
    : deskFrameGeneration;
  const comma = src.indexOf(',');
  noteDeskFrameBytes(comma >= 0 ? Math.floor((src.length - comma - 1) * 0.75) : src.length, {
    cached,
    stamp: String(t || '')
  });
  const img = new Image();
  img.onload = () => {
    if (!deskCallbackCurrent(gen)) return;
    drawDeskBitmap(img, t);
    if (cached) {
      const fx = cachedPollDrawEffect();
      deskNeedKeyframe = fx.deskNeedKeyframe;
      deskDeltaLastSeq = fx.deskDeltaLastSeq;
      deskDeltaKeySeq = fx.deskDeltaKeySeq;
      deskDeltaInFlight = fx.deskDeltaInFlight;
      deskNeedKeyframeSince = Date.now();
      return;
    }
    if (opts && opts.seq != null) {
      const seq = Number(opts.seq) || 0;
      const keySeq = Number(opts.keySeq != null ? opts.keySeq : opts.seq) || seq;
      if (seq > 0) {
        deskDeltaLastSeq = seq;
        deskDeltaKeySeq = keySeq || seq;
        deskNeedKeyframe = false;
        deskNeedKeyframeSince = 0;
        deskDeltaInFlight = false;
      }
    } else {
      deskNeedKeyframe = false;
  ...
  img.onerror = () => {
    ...
    const hint = document.getElementById('deskHint');
    const stage = document.getElementById('deskStage');
    if (stage) stage.classList.remove('has-frame', 'has-canvas');
    if (hint) hint.textContent = '画面数据异常，继续等待下一帧…';
    deskNeedKeyframe = true;
    if (!deskNeedKeyframeSince) deskNeedKeyframeSince = Date.now();
    deskDeltaInFlight = false;
  };
  img.src = src;
  return true;
}
function showDeskFrame(image, t, opts) {
  return showDeskCanvasFromData(image, t, opts);
}
function applyDeskDelta(msg) {
  const canvas = document.getElementById('deskCanvas');
  if (!canvas || !msg || deskNeedKeyframe) {
    if (deskNeedKeyframe && !deskNeedKeyframeSince) deskNeedKeyframeSince = Date.now();
    return false;
  }
  ...
  deskDeltaInFlight = true;
  deskDeltaLastSeq = seq;
  let bytes = 0;
  let pending = tiles.length;
  let failed = false;
  const stamp = String(msg.t || '');
  const finish = () => {
    if (!deskCallbackCurrent(gen)) return;
    deskDeltaInFlight = false;
    if (failed) {
    ...
    deskDeltaW = w;
    deskDeltaH = h;
    noteDeskFrameBytes(bytes, { cached: false, stamp });
    const stage = document.getElementById('deskStage');
    const hint = document.getElementById('deskHint');
    if (stage) {
      stage.classList.add('has-frame', 'has-canvas', 'is-live');
  ...
  tiles.forEach(tile => {
    ...
    let src = String(tile.image || '').trim();
    if (!src || tw < 1 || th < 1) {
      failed = true;
      pending -= 1;
      if (pending <= 0) finish();
    ...
    const comma = src.indexOf(',');
    if (comma >= 0) bytes += Math.floor((src.length - comma - 1) * 0.75);
    const img = new Image();
    img.onload = () => {
      if (!deskCallbackCurrent(gen)) return;
    ...
    if (data && data.image) {
      ...
      if (!shouldApplyPollFrame({
        ...
        updateDeskStaleFromAges({
          ...
          shotAgeSec: data.ageSec,
          clientOnline: data.clientOnline,
        });
        return;
      }
      const stamp = String(data.t || '');
      showDeskFrame(data.image, stamp, { cached: true });
      updateDeskStaleFromAges({
        liveAgeMs: deskLastLiveFrameAt ? (Date.now() - deskLastLiveFrameAt) : 99999,
        shotAgeSec: data.ageSec,
        clientOnline: data.clientOnline,
...
function startDeskPoll(clientId, session) {
  stopDeskPoll();
  void pollDeskLatest(clientId, session);
...
function deskControlEnabled() {
  const cb = document.getElementById('deskEnableControl');
  return !!(cb && cb.checked);
}
/** Map pointer to normalized 0..1 coords inside object-fit:contain content (not letterbox bars). */
function deskNormFromClient(clientX, clientY) {
  const video = document.getElementById('deskVideo');
  const canvas = document.getElementById('deskCanvas');
  const stage = document.getElementById('deskStage');
  let el = null;
  let iw = 1;
  let ih = 1;
  if (stage && stage.classList.contains('has-video') && video) {
    el = video;
    iw = video.videoWidth || 1;
    ih = video.videoHeight || 1;
  } else if (stage && stage.classList.contains('has-canvas') && canvas) {
    el = canvas;
    iw = canvas.width || 1;
    ih = canvas.height || 1;
  } else if (video) {
...
function deskControlMouse(e) {
  if (!deskControlEnabled()) return;
  const pt = deskNormFromClient(e.clientX, e.clientY);
  if (!pt) return;
  let evType = 'move';
  if (e.type === 'mousedown') evType = 'down';
  else if (e.type === 'mouseup') evType = 'up';
  deskSendControl({ type: 'mouse', event: evType, button: e.button, x: pt.x, y: pt.y, buttons: e.buttons });
}
function deskControlWheel(e) {
  if (!deskControlEnabled()) return;
  e.preventDefault();
  deskSendControl({ type: 'mouse', event: 'wheel', deltaY: e.deltaY, deltaX: e.deltaX });
}
function deskControlKey(e) {
  if (!deskControlEnabled()) return;
  const stage = document.getElementById('deskStage');
  if (!stage || !stage.classList.contains('is-live')) return;
  const t = e.target;
...
function bindDeskControl() {
  ['deskVideo', 'deskCanvas'].forEach(id => {
    const el = document.getElementById(id);
    if (!el || el.__deskCtrl) return;
    el.__deskCtrl = true;
    el.addEventListener('mousemove', deskControlMouse);
    el.addEventListener('mousedown', deskControlMouse);
    el.addEventListener('mouseup', deskControlMouse);
    el.addEventListener('wheel', deskControlWheel, { passive: false });
    ...
    el.addEventListener('click', e => {
      ...
      const stage = document.getElementById('deskStage');
      if (!stage || !stage.classList.contains('is-live')) return;
      setDeskFullscreen(!stage.classList.contains('fs'));
    });
  });
  if (!window.__deskKeyCtrl) {
    window.__deskKeyCtrl = true;
    document.addEventListener('keydown', deskControlKey);
    document.addEventListener('keyup', deskControlKey);
  }
}
async function startDesktop(clientId) {
  if (!clientId) return;
  const intent = ++deskIntentEpoch;
  state.desktopSelectedId = clientId;
  if (deskConnecting) {
    deskQueuedStartClientId = clientId;
    return;
  }
  deskConnecting = true;
  ...
  updateDesktopInfo(clientId);
  patchDesktopPage(state.overview);
  bindDeskControl();
}


async function stopDesktop(clearSelection, preserveIntent) {
  if (!preserveIntent) {
    deskIntentEpoch++;
    deskQueuedStartClientId = '';
  }
  ...
  const cid = state.desktopClientId;
  const sid = deskWebRTCSessionId;
  state.desktopClientId = '';
  stopDeskWebRTC();
  stopDeskSocketOnly();
  stopDeskPoll();
  ...
  if (sid) {
    try {
      await api('/api/desktop/webrtc/stop', { method: 'POST', body: JSON.stringify({ desktopSessionId: sid, clientId: cid }) });
    } catch (_) {}
  }
  if (cid) {
    try { await api('/api/desktop/stop', { method: 'POST', body: JSON.stringify({ clientId: cid }) }); } catch (_) {}
  }
  if (clearSelection === true) state.desktopSelectedId = '';
  const hint = document.getElementById('deskHint');
  clearDeskFrame();
...
function bindDeskFullscreen() {
  const closeBtn = document.getElementById('deskFsClose');
  if (closeBtn && !closeBtn.__fs) {
    closeBtn.__fs = true;
    closeBtn.onclick = e => { e.preventDefault(); setDeskFullscreen(false); };
  }
  bindDeskControl();
}
function updateDesktopInfo(clientId) {
  const el = document.getElementById('deskInfoBody');
  if (!el) return;
  const row = ((state.overview && state.overview.online) || []).find(r => r.clientId === clientId);
  if (!row) { el.innerHTML = '<p class="muted">客户端离线或不存在</p>'; return; }
  el.innerHTML =
    '<p><b>' + escHtml(displayAccount(row)) + '</b></p>' +
    '<p class="muted">ID<br/><span class="mono">' + escHtml(row.clientId) + '</span></p>' +
    '<p class="muted">IP<br/>' + escHtml(row.ip || '—') + '</p>' +
    '<p class="muted">版本<br/>' + escHtml(row.version || '—') + '</p>' +
    '<p class="muted">计划<br/>' + escHtml(displayPlan(row)) + '</p>' +
    '<p style="margin-top:12px">' + (row.allowed !== false ? '<span class="badge badge-ok">允许运行</span>' : '<span class="badge badge-deny">禁止运行</span>') + '</p>';
}
...
function renderPage(routeId) {
  ...
  if (['control','announcements','logs','desktop','formula-stats','wx-monitor','wx-groups'].includes(routeId)) {
    fetchOverview(true).catch(handleAuthError);
  }
}
...
function renderWxFeatureShell(routeId) {
  ...
  return '<div class="stack">' +
    ...
    '<button class="btn btn-secondary btn-sm" id="wxGoDesktop">打开远程桌面</button>' +
...
function renderDashboardShell() {
  return '<div class="page-grid">' +
    ...
    '<button class="btn btn-primary" id="dashGoDesktop">打开远程桌面</button>' +
...
function renderDesktopShell() {
  return '<div class="desktop-layout"><div class="card desktop-list" id="deskList"></div>' +
    '<div class="card desktop-stage" id="deskStage">' +
    '<video id="deskVideo" autoplay playsinline muted></video>' +
    '<canvas id="deskCanvas"></canvas>' +
    '<img id="deskImg" class="is-empty" alt="桌面" style="display:none"/>' +
    '<button class="desktop-fs-close" id="deskFsClose">×</button>' +
    '<span class="muted" id="deskHint">选择客户端后点击「开始查看」</span>' +
    '<div id="deskStaleBanner">画面已停住（缓存）</div>' +
    '<div class="desktop-stats" id="deskStats">' +
    ...
    '<button class="btn btn-danger" id="deskStop">停止查看</button>' +
    '<select id="deskQuality" class="input" style="width:auto;flex:1;min-width:110px">' +
    '<option value="auto">画质：自动</option><option value="smooth">画质：流畅</option><option value="clear">画质：清晰</option></select></div>' +
    '<label style="display:flex;align-items:center;gap:8px;margin-top:10px;font-size:13px;cursor:pointer">' +
    '<input type="checkbox" id="deskEnableControl"/> 启用远程控制（鼠标/键盘）</label></div></div>';
}
...
function bindClientsTableEvents(tbody) {
  ...
  tbody.querySelectorAll('.desk-btn').forEach(el => {
    el.onclick = () => { state.desktopSelectedId = el.dataset.cid; navigate('desktop'); };
  });
...
async function selectDesktopClient(cid) {
  if (!cid) return;
  const intent = ++deskIntentEpoch;
  deskQueuedStartClientId = '';
  if (state.desktopClientId && state.desktopClientId !== cid) {
    await stopDesktop(false, true);
  }
  if (intent !== deskIntentEpoch) return;
  state.desktopSelectedId = cid;
  patchDesktopPage(state.overview);
  updateDesktopInfo(cid);
  const hint = document.getElementById('deskHint');
  if (hint && !state.desktopClientId) hint.textContent = '已选择客户端，点击「开始查看」';
}
function patchDesktopPage(data) {
  ...
  const rows = data.online || [];
  list.innerHTML = rows.length ? rows.map(r => {
    const active = state.desktopSelectedId === r.clientId;
    return '<div class="desktop-client' + (active ? ' active' : '') + '" data-cid="' + escHtml(r.clientId) + '">' +
      '<b>' + escHtml(displayAccount(r)) + '</b><div class="muted mono">' + escHtml(r.clientId) + '</div><div class="muted">' + escHtml(r.ip || '') + '</div></div>';
  }).join('') : '<p class="muted">无在线客户端</p>';
  list.querySelectorAll('.desktop-client').forEach(el => {
    el.onclick = () => { selectDesktopClient(el.dataset.cid).catch(alert); };
  });
  if (state.desktopSelectedId) updateDesktopInfo(state.desktopSelectedId);
  bindDeskFullscreen();
}
...
function bindPageEvents(routeId) {
  ...
  if (routeId === 'desktop') {
    document.getElementById('deskStart').onclick = () => {
      if (!state.desktopSelectedId) return alert('请先选择客户端');
      startDesktop(state.desktopSelectedId).catch(alert);
    };
    document.getElementById('deskStop').onclick = () => stopDesktop(false);
    bindDeskFullscreen();
  }
  if (routeId === 'releases') {
